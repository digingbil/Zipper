...
class Zipper {
	...
	protected $excludes = [
        ...
        'wp-content/updraft',
        'wp_content/uploads/backupbuddy_backups',
	...
	const BKP_FILE_NAME_PREFIX = '__zipper__';
	private $error = '';
	private $isJoomla = false;
	...
	public function __construct($dir = __DIR__){
		...
		if( file_exists($dir) && is_dir($dir) ) {
			...
			throw new Exception('Directory passed is not valid!');
			
		}

		$this->env = $this->getEnv();

		$this->isJoomla = $this->isItJoomla();
		$this->isWordPress = $this->isItWordPress();
	...
	public function generateFileName($ext = null) {
		...
		if( !$ext ) {
			throw new Exception('No file extension passed.');
		}

		$stamp = date('d-m-Y_His');

		return self::BKP_FILE_NAME_PREFIX.$stamp.'.'.$ext;
	...
	public function getEnv() {
		...
		return [
			...
			'system' => $this->isEnabled('system'),
			'shell_exec' => $this->isEnabled('shell_exec'),
			'exec' => $this->isEnabled('exec'),
			'passthru' => $this->isEnabled('passthru'),
			'proc_open' => $this->isEnabled('proc_open'),
			'popen' => $this->isEnabled('popen')
		];
	...
	public function run( $include_db = false ) {

		if( $include_db ) {

			if( !$this->isJoomla && !$this->isWordPress ) {
				throw new Exception('DB Dump will work only on Joomla and WordPress databases.');
			}

			$dumped = $this->dbDump();

			if( !$dumped ) {
				throw new Exception('Error dumping database');
			}
		}
		
		if( $this->env['ZipArchive'] ) {
		...
		} elseif ( $this->env['exec'] ) {
			/* On some servers even tho the exec() and shell_exec() are enabled
			 * it still isn't possible to run commands. They return null (shell_exec) or 
			 * 126/127 exit codes (exec()). So let's check that first with some simple command
			 */
			$could_run_commands = true;

			@exec( 'ls', $output, $return_val );
			if( $return_val == 126 || $return_val == 127 ) {
				$could_run_commands = false;
			}

			if( $could_run_commands ) {

				$zipped = $this->tarGzIpit();
			...
			//TODO: Include PclZip Class
			throw new Exception('The Zipper could not run in this server environment.');
	...
	public function zipItUp() {

		$this->zip_file_name = rtrim( $this->dir, '/' ).'/'.$this->generateFileName('zip');

		//Check excludes
		$excludes_fnames = [];
		foreach ( $this->excludes as $exclude ) {
			$excludes_fnames[] = basename($exclude);
		}
		
		$zip = new ZipArchive();
		$res = $zip->open( $this->zip_file_name, ZipArchive::CREATE | ZipArchive::OVERWRITE );
		...
		foreach ($files as $name => $file) {
			...
			if( 
				$this->fileShouldBeRemoved( $file ) || 
				in_array( basename( $file->getRealPath() ), $excludes_fnames)
			) {
	    		continue;
	    	}
	...
	public function tarGzIpit() {

		$excludes_string = '';

		$this->zip_file_name = $this->generateFileName('tar.gz');

		foreach( $this->excludes as $exclude ){
			$excludes_string .= ' --exclude="'.'./'.$exclude.'"';
        }

		foreach( $this->findOldBackups() as $old_bkp ){
			$excludes_string .= ' --exclude="'.'./'.ltrim($old_bkp,'/').'"';
        }

		$command = 'tar '.$excludes_string.' -zcvf '.rtrim( $this->dir, '/' ).'/'.$this->zip_file_name.' -C '.$this->dir.' . ';
		$command = escapeshellcmd( $command );

		//Blocking call, not in background ie:  $cmd . ' > /dev/null &'
		exec( $command, $output, $return_val );

		// Return vals from tar:
	...
	public function findOldBackups() {
		...
		foreach ($files as $file) {
		    if( $this->fileShouldBeRemoved( $file ) ) {
	    		$file = str_replace( rtrim( $this->dir, '/' ), '', $file);
	    		$olds[] = $file;
	    	}
		}
	...
	public function fileShouldBeRemoved( $file ) {
		if(
			false === strpos($file, '/.') &&
			pathinfo($file, PATHINFO_EXTENSION) != 'sql' &&
			(
				pathinfo($file, PATHINFO_EXTENSION) == 'jpa' || //Joomla Akeeba Backups
				strpos( basename( $file->getRealPath() ), self::BKP_FILE_NAME_PREFIX ) !== false ||
				basename( $file->getRealPath() ) === __CLASS__.'.php'
			)
	...
	public function getCMSConfig() {
		...
		if( $this->isJoomla ) {
			...
			$config_file = rtrim( $this->dir,'/').'/configuration.php';
			include_once($config_file);
			$jconfig = new \JConfig;
			$config = get_object_vars($jconfig);

			return (object) $config;

		} elseif ( $this->isWordPress ) {

			$config_file = rtrim( $this->dir,'/').'/wp-config.php';
			include_once($config_file);
			$config = array(
				'db' => DB_NAME,
				'user' => DB_USER,
				...
				'dbprefix' => $table_prefix ? $table_prefix : ''
			);
	...
	public function isItJoomla() {
		if (
			...
			is_dir( rtrim( $this->dir,'/').'/modules') &&
			is_dir( rtrim( $this->dir,'/').'/plugins') &&
			is_dir( rtrim( $this->dir,'/').'/components') &&
			is_dir( rtrim( $this->dir,'/').'/components/com_content') &&
			is_dir( rtrim( $this->dir,'/').'/administrator') &&
			is_dir( rtrim( $this->dir,'/').'/administrator/components') &&
			is_dir( rtrim( $this->dir,'/').'/administrator/components/com_content') &&
			is_dir( rtrim( $this->dir,'/').'/templates' ) &&
			file_exists( rtrim( $this->dir,'/').'/configuration.php' )
	...
	public function isItWordPress() {
		...
		if (
			...
			is_dir( rtrim( $this->dir,'/').'/wp-content') &&
			is_dir( rtrim( $this->dir,'/').'/wp-admin') &&
			is_dir( rtrim( $this->dir,'/').'/wp-includes') &&
			is_dir( rtrim( $this->dir,'/').'/wp-content/plugins') &&
			is_dir( rtrim( $this->dir,'/').'/wp-content/themes') &&
			file_exists( rtrim( $this->dir,'/').'/wp-login.php' ) && 
			file_exists( rtrim( $this->dir,'/').'/wp-load.php' ) && 
			file_exists( rtrim( $this->dir,'/').'/wp-config.php' )
	...
	public function dbDump() {
		...
		$app_config = $this->getCMSConfig();

		if( !$app_config ) {
			$this->setError('Could not get the DB config params!');
			return false;
		}

		$db_name = $app_config->db;
		$db_user = $app_config->user;
		$db_pass = $app_config->password;
		$db_host = $app_config->host;

		$filename = rtrim( $this->dir,'/') .'/'.$this->generateFileName('sql');
		...
		try {
			$dump = new Mysqldump('mysql:host='.$db_host.';dbname='.$db_name, $db_user , $db_pass, $settings);
			$dump->start( $filename );
			...
			$this->setError( 'Error dumping database');
	...
	public function serveFile( $file = null ) {
		...
		if( $file ) {
			...
			$is_targz = strpos( $file, '.tar.gz' ) !== false;
			...
			throw new Exception( 'Error downloading the archive. You will have to manually get it from the server, ie: FTP' );
			
		}

	}

	public function removeZipperFiles( $clean_session = true ) {
		
		$files = glob( rtrim($this->dir,'/').'/*'); // get all file names
		...
		foreach($files as $file){ 
			...
			if( is_file($file) ) {
				...
				if( strpos($basename, self::BKP_FILE_NAME_PREFIX) !== false ) {
					@unlink($file);
				}
		...
		if( $clean_session ) {
			if( isset($_SESSION[self::BKP_FILE_NAME_PREFIX]) ) {
				unset( $_SESSION[self::BKP_FILE_NAME_PREFIX] );
			}
		}
...
$submitted = isset( $_POST['zipitup'] );
$include_db = isset( $_POST['include_db'] );
$downloadit = isset( $_POST['downloadit'] );
$deleteit = isset( $_POST['deleteit'] );
$delete_zipper_files = isset( $_POST['delete_zipper_files'] );

if( $submitted ) {

	$zipped = $zipper->run( $include_db );

	if( $zipped ) {

		$data = [
			'file_name' => $zipper->zip_file_name,
			'success' => true,
			'error' => $zipper->getError()
		];
		$_SESSION[$zipper::BKP_FILE_NAME_PREFIX] = $data;
...
if( $downloadit ) {
	$file = $_SESSION[$zipper::BKP_FILE_NAME_PREFIX]['file_name'];
	$zipper->serveFile( $file );
}

if( $delete_zipper_files ) {
	$zipper->removeZipperFiles();
	header( 'Location: ' . $_SERVER['PHP_SELF'] );
}

$zipper_data = isset( $_SESSION[$zipper::BKP_FILE_NAME_PREFIX] ) ? $_SESSION[$zipper::BKP_FILE_NAME_PREFIX] : null;
?>
<!DOCTYPE html>
<html lang="en">
<head>
	...
	<style>
		*{margin:0;padding:0;box-sizing: border-box;}
		body{font-family: Helvetica, Arial, sans-serif;font-size: 14px;color: #141414;line-height:1.6;padding-top: 50px;}
		.wrapper{width:1100px;margin:0 auto;max-width: 90%;border:1px solid #ccc;padding:30px;box-shadow: 0px 0px 15px rgb(0 0 0 / 10%);}
		h1{color:#fd4241;margin-bottom: 20px;}
		p, .output {margin-bottom: 20px;}
		p.alert{border: 2px solid cornflowerblue;padding: 10px 20px;font-style: italic;}
		p.error{border: 2px solid crimson;padding: 10px 20px;font-style: italic;}
		pre{margin-bottom: 20px;}
		pre, .pre{background: #f0f0f0;color:000;font-family: Consolas, Robotto, "Source Code Pro", monospace;font-weight:700;}
		.form-row{margin-bottom: 20px;}
		button{padding: 10px 22px;}
		.output{border: 2px solid limegreen;padding: 10px 20px;}
		.output form, .output form button{display: inline;}
		hr{border: 0;height: 1px;background-image: linear-gradient(to right, rgba(0, 0, 0, 0), rgba(0, 0, 0, 0.75), rgba(0, 0, 0, 0));margin-bottom: 20px;}
	</style>
</head>
<body>
	<div class="wrapper">
		<div class="inner">
			<h1>Zipper.</h1>
			<p>With this tool you can make a full site backups. Particularly useful for Joomla! or WordPress sites, although you can use it on any site. <br> Based on your server config, it will automatically decide which archive format it will use: ZIP or TAR.GZ.</p>
			<p>Directory:</p>
			<pre><?php echo __DIR__ ; ?></pre>
			<p class="alert">Please make sure you wait for the script to finish. It can take a while, depending on your site's size! </p>
			<form action="" method="post" id="zipper-form">
				<div class="form-row">
					<input type="checkbox" name="include_db" id="include_db"> <label for="include_db"><strong>Include the DB in the backup?</strong> <small>(Works only with Joomla! and WordPress!)</small></label>
				</div>
				<div class="form-row">
					<button id="sbmt-btn" type="submit">Start Zipping.</button>
				</div>
				<input type="hidden" name="zipitup" value="1">
			</form>

			<?php if( $zipper_data && !empty( $zipper_data['success'] ) ) : ?>
				<div class="output">
					Archive created at: <span class="pre"><?php echo $zipper_data['file_name']; ?></span>. <form action="" method="post"><button type="submit" name="downloadit">Download.</button></form>
				</div>
			<?php endif; ?>

			<?php if( $zipper_data && isset( $zipper_data['success'] ) &&  $zipper_data['success'] === false ) : ?>
				<p class="error">
					Problems encountered during archive creation: <?php echo '<span class="pre">'.$zipper_data['error'].'</span>' ?>
				</p>
			<?php endif; ?>

			<?php if( $zipper_data ) : ?>
				<hr>
				<form action="" id="delete-zipper-files-form" method="post">
					<div class="form-row">
						<button id="sbmt-btn" type="submit">Delete all zipper archive files.</button>
					</div>
					<input type="hidden" name="delete_zipper_files" value="1">
				</form>
			<?php endif; ?>
			...
			<form action="" id="delete-form" method="post">
				<div class="form-row">
					<button id="sbmt-btn" type="submit">Delete the Zipper.php from the server.</button>
				</div>
...
<script>
	(function(){
		...
		var frm = document.getElementById('zipper-form'),
			sbmt = document.getElementById('sbmt-btn');
			sbmt.disabled = false;
